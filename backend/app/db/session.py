...
async def init_db() -> None:
    ...
    def _work():
        ...
        with engine.begin() as conn:
            ...
            conn.execute(text("ALTER TABLE deployments ADD COLUMN IF NOT EXISTS feishu_webhook_url text"))
            conn.execute(text("ALTER TABLE deployments ADD COLUMN IF NOT EXISTS feishu_secret text"))
            conn.execute(text("ALTER TABLE deployments ADD COLUMN IF NOT EXISTS notify_on_success boolean NOT NULL DEFAULT true"))
            conn.execute(text("ALTER TABLE deployments ADD COLUMN IF NOT EXISTS notify_on_failed boolean NOT NULL DEFAULT false"))
